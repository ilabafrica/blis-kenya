	...
	foreach($resultset as $record)
	{
		...
		?> id="<?php echo $test->testId; ?>">
			...
			if($test_status == Specimen::$STATUS_STARTED){
				...
				echo '</span></td>';
				echo '
			<td id=action'.$test->testId.' style="width:130px;"><a href="javascript:fetch_test_result_form('.$quote.$test->testId.$quote.',0);" title="Click to Enter Results for this Specimen" class="btn yellow mini">
